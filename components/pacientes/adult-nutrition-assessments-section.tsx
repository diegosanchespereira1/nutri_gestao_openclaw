...
function AdultNutritionAssessmentHistoryCard({
  row,
}: {
        ...
        <div className="space-y-4 border-t border-border bg-card px-4 py-4 text-sm">
          ...
          {(row.kcal_per_kg != null || row.ptn_per_kg != null) ? (
            <div>
              ...
              <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-3 text-sm sm:grid-cols-2">
                <DataItem
...
export async function AdultNutritionAssessmentsSection({
  patientId,
  defaultAge,
}: {
  patientId: string;
  ...
  const { rows } = await loadAdultNutritionAssessmentsForPatient(patientId);
    ...
    <div className="space-y-6" aria-label="Avaliações nutricionais adultos">
      ...
      <div className="border-t border-border pt-6">
        <h3 className={formSectionLegendClass}>
          Histórico de avaliações (adultos)
        </h3>
        {rows.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            Ainda não há avaliações neste modelo. Use o formulário acima para o
            primeiro registro.
          </p>
        ) : (
          <ul className="space-y-2" aria-label="Histórico de avaliações adultos">
            {rows.map((r) => (
              <AdultNutritionAssessmentHistoryCard key={r.id} row={r} />
            ))}
          </ul>
        )}
      </div>
