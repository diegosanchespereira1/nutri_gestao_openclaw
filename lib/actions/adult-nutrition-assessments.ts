...
export async function loadAdultNutritionAssessmentsForPatient(
  patientId: string,
): Promise<{ rows: AdultNutritionAssessmentRow[] }> {
  ...
  return { rows: data as AdultNutritionAssessmentRow[] };
}

export async function createAdultNutritionAssessmentAction(
  _prev: AdultNutritionAssessmentFormResult | undefined,
  formData: FormData,
): Promise<AdultNutritionAssessmentFormResult> {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  ...
  const { error } = await supabase.from("patient_adult_nutrition_assessments").insert({
